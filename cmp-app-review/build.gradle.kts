...
kotlin {
    ...
    // ========================================================================
    android {
        namespace = "com.mobilebytelabs.kmptoolkit.appreview"
        compileSdk =
    ...
    sourceSets {
        ...
        // tvOS and watchOS behave identically here — no SKStoreReviewController, store page only —
        // so they share one actual instead of two copies that must be kept in step.
        val appleStoreOnlyMain = create("appleStoreOnlyMain").apply { dependsOn(getByName("commonMain")) }
        listOf("tvosMain", "watchosMain").forEach { getByName(it).dependsOn(appleStoreOnlyMain) }

        // Desktop JVM, Linux, browser and Node all resolve to a web listing opened by cmp-open-url.
        // Windows is deliberately NOT here: it has its own ms-windows-store:// deep link.
        val webFallbackMain = create("webFallbackMain").apply { dependsOn(getByName("commonMain")) }
        listOf("jvmMain", "linuxMain", "jsMain", "wasmJsMain").forEach {
            getByName(it).dependsOn(webFallbackMain)
        }
        ...
        androidMain.dependencies {
            // Google Play In-App Review. Android-only by nature; every other target either has its
            // own native API or falls back to the store listing.
...
mavenPublishing {
    ...
    pom {
        name = "CMP App Review"
        description =
            "Cross-platform app review for Kotlin Multiplatform — native in-app review on Android/iOS/macOS, " +
            "store-listing fallback everywhere else via cmp-open-url"
        inceptionYear = "2026"
