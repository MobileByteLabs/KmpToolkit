---
module: cmp-observe-firebase
artifact: io.github.mobilebytelabs:cmp-observe-firebase
version: UNKNOWN
package: com.mobilebytelabs.kmptoolkit.observe.firebase
...
# cmp-observe-firebase — Development

> Single source of truth for development state of `cmp-observe-firebase` (KMP library module). Per RULE-LIB-DEVELOPMENT-MD-001.
...
|----------|---------|-----------------|-------|-------|----------|
| `io.github.mobilebytelabs:cmp-observe-firebase` | `com.mobilebytelabs.kmptoolkit.observe.firebase` | `UNKNOWN` | [Central](https://central.sonatype.com/artifact/io.github.mobilebytelabs/cmp-observe-firebase) | 2026-09-13 | experimental |

**Module purpose (one paragraph):** <!-- AUTHOR: WIP — initial draft from 2026-09-13. One-paragraph module purpose (≤200 words). Seed from idea-layer/cmp-observe-firebase/SPEC.md if present. -->
...
|------|--------|----|---------|-------|
| (no open PRs labeled `cmp-observe-firebase` — refresh via `gh pr list --label cmp-observe-firebase` then re-run scan) | — | — | — | — |
...
|-------------|--------|---------|
| T0 (Crashlytics attribution) | enabled | custom_key: `library:cmp-observe-firebase@UNKNOWN` (set on init by FirebaseCrashlyticsAttributionHook) |
...
tiers:
  ...
  T1: enabled
  T2: opted-out
custom_key_format: "library:cmp-observe-firebase@UNKNOWN"
event_schema: []  # populate when T2 enabled — see library-runtime-observability epic AC #12-13
consumer_opt_in: "lib-integrate.properties#cmp-observe-firebase.observability_opt_in"
```

**Consumer opt-in:** controlled via `cmp-observe-firebase.observability_opt_in=true` in consumer's `lib-integrate.properties`.
