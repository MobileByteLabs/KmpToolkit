...
class RemoteConfigEvaluator(
    ...
    private fun isDismissed(
        ...
        return (serverImpressions[config.id]?.dismissed ?: false) ||
...
private fun String.parseIsoToMs(): Long? {
    return try {
        ...
        val year = d[0].toInt(); val month = d[1].toInt(); val day = d[2].toInt()
        val hour = t[0].toInt(); val min = t[1].toInt(); val sec = t.getOrNull(2)?.toIntOrNull() ?: 0
