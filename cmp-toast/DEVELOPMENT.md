---
module: cmp-toast
artifact: io.github.mobilebytelabs:cmp-toast
version: UNKNOWN
...
# cmp-toast — Development

> Single source of truth for development state of `cmp-toast` (KMP library module). Per RULE-LIB-DEVELOPMENT-MD-001.
...
|----------|---------|-----------------|-------|-------|----------|
| `io.github.mobilebytelabs:cmp-toast` | `com.mobilebytelabs.kmptoolkit.toast` | `UNKNOWN` | [Central](https://central.sonatype.com/artifact/io.github.mobilebytelabs/cmp-toast) | 2026-05-30 | experimental |

**Module purpose (one paragraph):** <!-- AUTHOR: WIP — initial draft from 2026-05-30. One-paragraph module purpose (≤200 words). Seed from idea-layer/cmp-toast/SPEC.md if present. -->
...
|------|--------|----|---------|-------|
| (no open PRs labeled `cmp-toast` — refresh via `gh pr list --label cmp-toast` then re-run scan) | — | — | — | — |
...
|-------------|--------|---------|
| T0 (Crashlytics attribution) | enabled | `custom_key: library:cmp-toast@<version>` — set by `FirebaseCrashlyticsAttributionHook` (`cmp-observe-firebase`) |
| T1 (init + version health) | n/a | not applicable — this module has no initialisation step of its own |
| T2 (lifecycle events) | enabled | `toast_shown` |
