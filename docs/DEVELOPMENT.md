---
title: "Docs authoring guide (KmpToolkit)"
---

# Docs authoring guide (KmpToolkit-specific)

KmpToolkit extensions to the canonical authoring guide that lives in
[`DEVELOPMENT-TEMPLATE.md`](DEVELOPMENT-TEMPLATE.md) — **read that first.**
The conventions below assume the generic baseline (special files, style
guide, mkdocs pipeline) and only document what's specific to this 21-module
multi-surface library.

This file is **owned by this repo**. Unlike `DEVELOPMENT-TEMPLATE.md` (which
is sync'd from `mbl-library-template-kmp`), this one is hand-authored and
captures KmpToolkit's distinctive shape:

- A **third surface**: Dokka API reference bundled inside `-javadoc.jar`
- **Three content types**: narrative, per-module pages, cookbook recipes
- **Cookbook recipe enforcement**: CI-checked line/format constraints
- **Legacy directory migration**: 14 excluded `docs/<module>/` subdirs
  awaiting lift into the current structure

## The third surface (in addition to mkdocs + wiki)

| Surface | Source | Lands at |
|---------|--------|----------|
| **mkdocs site** (canonical) | `docs/**`, `mkdocs.yml` | `https://mobilebytelabs.github.io/KmpToolkit/` |
| **Dokka API reference** (KmpToolkit-specific) | Kotlin `///` KDoc in source | Bundled inside each module's `-javadoc.jar` on Maven Central |
| **GitHub Wiki** | `docs/**` mirrored | `https://github.com/MobileByteLabs/KmpToolkit/wiki/<basename>` |

The Dokka HTML is built per-module via the convention plugin at
`build-logic/convention/src/main/kotlin/DokkaConventionPlugin.kt`. Each
`cmp-*/build.gradle.kts` applies it via
`id("io.github.mobilebytelabs.kmptoolkit.dokka")` + wires the output into
`vanniktech.mavenPublish` via
`JavadocJar.Dokka("dokkaGeneratePublicationHtml")`. The result ships
inside every module's `-javadoc.jar` artifact on Maven Central; users open
the HTML from the Maven Central UI.

## The three content types

### 1. Narrative docs (`docs/index.md`, `docs/getting-started.md`)

Top-of-funnel pages — same as the generic guide describes for any library.
Authored prose, full freedom on structure.

### 2. Per-module pages (`docs/modules/cmp-*.md`) — KmpToolkit-specific

One page per published module. **Two flavors, both managed in `mkdocs.yml`
under `nav: Modules:`:**

- **README-embedded** (10 modules today, those with `cmp-*/README.md`):
  uses the `mkdocs-include-markdown-plugin` Liquid-style directive to
  embed the module's source-tree README. See any existing
  `docs/modules/cmp-network-monitor.md` for live syntax.
  **Don't duplicate** the README content into the `docs/modules/` page —
  let the include do its job.
- **Placeholder** (11 modules without README.md yet): a minimal page
  noting "shipped; full docs coming" + GitHub source link + a one-line
  API reference note pointing at the `-javadoc.jar` on Maven Central.

Whichever flavor: always include the API reference note.

When a module gains a `cmp-*/README.md`, convert its
`docs/modules/cmp-*.md` placeholder to the embedded form **in the same
PR**.
...
links that resolve only in the GitHub UI. They live in `docs/` for
backward compat but are excluded from the mkdocs build via
...
**Don't add new content to those directories.** Lift content into one of
the three current surfaces:

- Module-level reference → write `cmp-*/README.md` (becomes source for
  `docs/modules/cmp-*.md` via include-markdown)
...
## Agent quick reference — KmpToolkit-specific

Generic recipes (add a page, add an asset, override brand colors, test,
deploy) live in `DEVELOPMENT-TEMPLATE.md`. Below are operations specific
to KmpToolkit's three-content-type structure.
...
## Invariants — KmpToolkit-specific

Generic invariants (`Home.md` ↔ `index.md` sync, mkdocs nav for new pages,
etc.) live in `DEVELOPMENT-TEMPLATE.md`.

| If you change… | Also update… | Why |
|----------------|--------------|-----|
| New `cmp-<name>/` Gradle module | (1) `docs/modules/cmp-<name>.md` (2) `mkdocs.yml` → `nav: Modules:` (3) `cmp-<name>/build.gradle.kts` Dokka plugin + JavadocJar config (4) `cmp-<name>/CHANGELOG.md` (5) root `CHANGELOG.md` entry | Otherwise module ships without docs, API ref, or release notes |
...
## Scaling rubric — KmpToolkit-specific

Generic scaling rubric (single-module → multi-module shape) lives in
`DEVELOPMENT-TEMPLATE.md`. KmpToolkit is sized for ≥ 15 modules with heavy
how-to content. Below: what to add as it scales further.
...
| **Multi-version docs needed** (e.g. v3 + v4 coexisting) | Add `mike` plugin for versioned docs. Significant ceremony; only adopt when users genuinely need v3 docs after v4 ships. |
...
## Validation commands — KmpToolkit-specific

Generic validation (strict build, link audit, site liveness probe) lives
in `DEVELOPMENT-TEMPLATE.md`. Below: audits specific to KmpToolkit's
cookbook + multi-module structure.
...
Run all the audits before opening a release PR to catch drift before
users see it.
