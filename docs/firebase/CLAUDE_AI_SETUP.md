# cmp-firebase — Claude AI Setup Guide

`/sync-firebase-analytics` is a verify-gated sync that wires `cmp-firebase` into a consuming KMP app. Run from anywhere in the project; Claude detects the active project and walks the gates.
...
/lib-sync cmp-firebase   # Same as /sync-firebase-analytics
```
...
|  Check: cmp-firebase:VERSION in libs.versions    |
...
### Version bump (cmp-firebase 1.0.0 → 1.1.0)
...
```kotlin
io.github.mobilebytelabs.kmptoolkit.firebase.analytics.*
...
io.github.mobilebytelabs.kmptoolkit.firebase.analytics.mp.*
