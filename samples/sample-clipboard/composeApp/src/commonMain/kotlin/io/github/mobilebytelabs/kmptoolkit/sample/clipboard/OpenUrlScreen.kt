...
private data class UrlDemoAction(
    val label: String,
    val emoji: String,
...
fun OpenUrlScreen() {
    ...
    val actions = remember {
        listOf(
            ...
            UrlDemoAction(
                label = "Email App",
                emoji = "\uD83D\uDCE7",
                description = "\"mailto:hi@example.com\".openWith(AppHint.EMAIL)",
                color = Color(0xFFBF360C),
            ) {
                resultLabel("EMAIL", "mailto:hi@example.com".openWith(AppHint.EMAIL))
            },
            UrlDemoAction(
                label = "Maps App",
                emoji = "\uD83D\uDDFA\uFE0F",
    ...
    Column(
        ...
        actions.forEach { urlAction ->
            UrlActionCard(action = urlAction, onClick = {
                scope.launch { lastResult = urlAction.action() }
            })
        }

        lastResult?.let { result ->
            Spacer(modifier = Modifier.height(4.dp))
            OpenUrlResultCard(result = result, onDismiss = { lastResult = null })
        }
...
@Composable
private fun UrlActionCard(action: UrlDemoAction, onClick: () -> Unit) {
    Card(
...
@Composable
private fun OpenUrlResultCard(result: String, onDismiss: () -> Unit) {
...
private fun resultLabel(hint: String, result: OpenUrlResult): String = when (result) {
    ...
    is OpenUrlResult.NoHandler -> "openWithApp($hint) → NoHandler"
